...
def parse(path):
    root=xml_root(path)
    out={'events':[],'attributes':[],'marks':[],'display':[],'layout':[],'measures':[],'issues':[],'notes':[]}
    ...
    for pi,part in enumerate(root.findall('part'),1):
        ...
        divisions=1;active={};spans={}
        for mi,m in enumerate(part.findall('measure'),1):
            loc={'part':pi,'measure':mi};cursor=F(0);last=None;extent=F(0);intervals={}
            for e in m:
                ...
                if e.tag=='attributes':
                    for a in e:
                        ...
                        elif a.tag not in ('staff-details','instruments'):
                    ...
                    if grace:issue(where,'Grace-note ordering and playback require manual review')
                    ...
                    pitch=e.find('pitch');rest=e.find('rest')
                    if pitch is None and rest is None:issue(where,'Unpitched or unsupported note');continue
                    spelling=None if rest is not None else [pitch.findtext('step'),str(F(pitch.findtext('alter','0'))),int(pitch.findtext('octave'))]
                    ...
                    if tm is not None:
                        ...
                        issue(where,'Tuplet ratio parsed, but tuplet display requires manual verification')
                    ...
                    note_allowed={'pitch','rest','duration','voice','type','dot','accidental','stem','staff','beam','notations','chord','tie','time-modification','grace'}
                    for ch in e:
                    ...
                    if nt is not None:
                        for n in nt:
                            ...
                            if n.tag in ('articulations','fermata','arpeggiate','non-arpeggiate'):
                                record('marks',where,kind=n.tag,value=atom(n))
                    ...
                    for d in e.findall('direction-type/*'):
                        ...
                        elif d.tag=='wedge':record('marks',dloc,kind='wedge',value=d.get('type'))
                        else:issue(dloc,'Unsupported direction '+d.tag)
                elif e.tag=='barline':
                    for b in e:
                        if b.tag=='bar-style':
                            if b.text!='regular':record('marks',loc,kind='barline',side=e.get('location','right'),value=b.text)
                        elif b.tag in ('repeat','ending'):record('marks',loc,kind=b.tag,side=e.get('location','right'),value=atom(b))
...
def audit(expected,actual,*,display=False,layout=False,merge=False):
    e=parse(expected);a=parse(actual);categories=['events','attributes','marks','measures']
    ...
    return {'schema':1,'expected_sha256':sha(expected),'actual_sha256':sha(actual),'mode':'condensed' if merge else 'voices','scope':categories,'passed':equality and not issues,'equal_in_checked_scope':equality,'status':'PASS' if equality and not issues else 'REVIEW_REQUIRED' if equality else 'FAIL','expected_events':len(e['events']),'actual_events':len(a['events']),'issues':issues,'differences':diffs,'limitations':['Equality does not establish that the expected score was correctly read from the source.','Content mode excludes explicit accidentals/stems/beams; enable --display for their comparison.','Layout, part names, timbre, playback realization, fonts and pixels are not certified.']}
